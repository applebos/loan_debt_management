...
이 프로젝트는 사용자가 대출 원금, 이자율, 기간 등 기본 정보를 입력하면, 다양한 상환 방식에 따른 월 납입금, 총 이자, 상환 스케줄을 한눈에 비교할 수 있는 웹 애플리케이션이며,
또한, 사용자의 현재 대출 조건이 시장 평균 대비 고금리일 경우, 더 나은 조건의 대환대출을 지능적으로 제안하여 사용자의 금융 비용 절감을 돕는 것을 목표로 함
---
...
- 실시간 금리 데이터 연동: 현재 상수로 관리되는 `TIER_1_AVG_RATE`를 외부 금융 API와 연동하여, 항상 최신 시장 금리 동향을 반영한 분석을 제공하도록 고도화할 계획입니다. 이를 통해 대환대출 제안의 정확도와 신뢰도를 극대화.
- 추가 상환 방식 지원: '체증식 분할상환' 등 더 다양한 대출 상환 방식을 계산기에 추가하여, 더 넓은 범위의 사용자들이 활용할 수 있도록 기능을 확장.
- 그래프 시각화: 계산 결과를 차트나 그래프 형태로 시각화하여 사용자가 상환 방식별 비용 차이를 더 직관적으로 이해할 수 있도록 개선.
